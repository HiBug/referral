<header class="box wrap-head">
		<div class="header">
			<div class="header-search">
				...
				<a class="header-logo" href="<?php echo SITE_PATH;?>">手赚网</a>
			        ...
			        <?php //print_r($c->categories);
                        foreach($c->categories as $a){
                            if($a['parent_id']=='0'){//只输出顶级分类
                                $style='';
                                if((intval($cid)>0 && ($a['cate_id'] == $cid || $a['cate_id']==$c->categories[$cid]['parent_id'])) || (isset($topid) && $topid == $a['cate_id'])) $style='class="on"';
