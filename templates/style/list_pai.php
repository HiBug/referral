...
<div class="box info-tab" id="info-tab">
						<a href="javascript:;" id="item-1" class="t-sel" onclick ="tab_info('item-1')">苹果手机赚钱软件排行</a>
						<a href="javascript:;" id="item-2" onclick="tab_info('item-2')">安卓手机赚钱软件排行</a>
</div>
</ul><div class="box part-2"></div>
			<ul class="app-list" id="app_url">
<div class="" id="tab-1">
      <?php $a = $c->get_recommend(array('area_id'=>4,'pagesize'=>10,'count'=>0,'rewrite'=>SITE_REWRITEURL,'where'=>'(area_type=0  )'));//print_r($a['list'][0]);?>
...
<div class="" id="tab-2"  style="display:none;">
      <?php $a = $c->get_recommend(array('area_id'=>5,'pagesize'=>10,'count'=>0,'rewrite'=>SITE_REWRITEURL,'where'=>'(area_type=3  )'));//print_r($a['list'][0]);?>
     <?php if(!empty($a['list'])){ foreach($a['list'] as $k => $v){?>
					<li class="box"  url="<?php echo 'http://'.$_SERVER['HTTP_HOST'].$v['surl'];?>" onclick="locaUrl(this)">
<a href="<?php echo $v1['surl'];?>"class="app-btn-down">下载</a>
						<a class="app-icon" href="<?php echo $v['surl'];?>">
